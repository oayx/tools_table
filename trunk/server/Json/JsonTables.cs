...
namespace YX
{
    ...
    public class JsonTables
    {
        ...
        /// 路径
        /// </summary>
        public static string _tablePath = "../data";
        /// <summary>
        /// 包括table字段的class
        /// </summary>
        ...
        public static void LoadTable<K, T>() where T : JsonTableRow<K>
        {
            ...
            System.Console.WriteLine("加载配置表:" + name);

            try
            {
                //加载
                string file_path = string.Format("{0}/{1}.json", _tablePath, name);
                string ta = ReadFile(file_path);
                if (string.IsNullOrEmpty(ta))
                {
                    System.Console.WriteLine("配置表加载失败:" + file_path);
                    return;
                }
                LoadFromData<K, T>(ta);
            }
            ...
            {
                System.Console.WriteLine(string.Format("配置表{0}读取失败:{1}", name, e.ToString()));
            }
        }
        /// <summary>
        /// 加载表
        ...
        private static JsonTable<K, T> LoadFromData<K, T>(string ta) where T : JsonTableRow<K>
        {
            ...
            if (dataSrc != null)
            {
                ...
                {
                    System.Console.WriteLine(string.Format("配置表构建失败:{0}", ex.ToString()));
                }
            }
            return null;
        }
        public static JsonDataSrc<K, T> Parse<K, T>(string ta) where T : JsonTableRow<K>
        {
            if (!string.IsNullOrEmpty(ta))
            {
                ...
                {
                    System.Console.WriteLine(string.Format("配置表解析失败:{0}", ex.ToString()));
                }
            }
            return null;
        }
        ...
        {
            System.Console.WriteLine("ReadConfig:" + fileName);
            try
            {
